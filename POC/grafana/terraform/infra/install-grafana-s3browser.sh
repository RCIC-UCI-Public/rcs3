...
aws s3 cp s3://backup-metrics-tfstate-dev/s3-browser/package.json package.json
...
aws s3 cp s3://backup-metrics-tfstate-dev/s3-browser/server.js server.js

# Create public directory and download HTML interface from S3
mkdir -p public
aws s3 cp s3://backup-metrics-tfstate-dev/s3-browser/public/index.html public/index.html
